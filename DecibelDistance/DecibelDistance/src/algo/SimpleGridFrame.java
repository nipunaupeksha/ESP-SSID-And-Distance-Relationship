...
public class SimpleGridFrame extends javax.swing.JFrame {
    ...
    private double[] sortedPreProcessedArr = {
    ...
    private double rawValuesArray[] = new double[9];
    private double processedDistances[] = new double[9];
    ...
    private void findDistances() {
        ...
        String str = "AA,9.90,7.28,7.62,7.28,2.83,3.61,7.62,3.61,4.24,BB";
        str = "AA,2.83,3.61,3.61,4.24,7.28,7.28,7.62,7.62,9.90,BB";
        //calculatePoint(str);
        //averageRatioValueSetter(str);
        //getTransmissionPowers(str);
        getAllTransmissionPowers();
    }
    ...
    private void getTransmissionPowers(String str) {
        double CONS = 1.0;
        String tempRawValues[] = str.split(",");
        double raw_transmission[] = new double[9];
        for (int i = 1; i < tempRawValues.length - 1; i++) {
            raw_transmission[i - 1] = Double.parseDouble(tempRawValues[i]);
        }

        for (int i = 0; i < raw_transmission.length; i++) {
            raw_transmission[i] = 20 * Math.log10(raw_transmission[i] / (double) CONS);
            //raw_transmission[i] = Math.round(raw_transmission[i]*100.0)/100.0;
        }
        /*
        for(double d:raw_transmission){
            System.out.println(d);
        }*/

    }

    private void getAllTransmissionPowers() {
        double CONS = 9870.023388987409;
        double radius = 0.0;
        double lgrad= 0.0;
        System.out.println("Strength \t \t Radius \t \t 20log(Radius)");
        for (int i = 0; i < 100; i++) {
            radius = CONS * Math.sqrt(Math.pow(10,(i/10.0)));
            lgrad = 20 * Math.log10(radius);
            System.out.println(i+"\t\t"+radius);
        }
    }

    /**/
    private void averageRatioValueSetter(String str) {
        String tempRawValues[] = str.split(",");
        for (int i = 1; i < tempRawValues.length - 1; i++) {
            rawValuesArray[i - 1] = Double.parseDouble(tempRawValues[i]);
        }
        ...
        //average the values accordingly and get the distances
        for (int i = 0; i < rawValuesArray.length; i++) {
            rawValuesArray[i] = averageRSSI(rawValuesArray[i]);
        }

        //then find the distances
        for (int i = 0; i < processedDistances.length; i++) {
            processedDistances[i] = assignDistances(rawValuesArray[i]);
        }

        //print the value set
        for (double d : rawValuesArray) {
            System.out.println(d);
        }
        System.out.println("");
        for (double d : processedDistances) {
            System.out.println(d);
        }
        //decibels to distance
        //range = constant * sqrt(transmit power)
        //range = constant * sqrt(10^(power in dBm/10)) -> RSSI gives power in dBm
        /*
        for (int i = 0; i < rawValuesArray.length; i++) {
            rawValuesArray[i] =Math.round(Math.sqrt(Math.pow(10, (rawValuesArray[i] / 10.0)))*100.0)/100.0;
        }
         */
        //print the value set
        /*
         ...
         */
        //find the minimum value in raw values
        /*
        double minimumValueOfRawValues = Double.MAX_VALUE;
        for (int i = 0; i < rawValuesArray.length; i++) {
            if (rawValuesArray[i] != 0 && rawValuesArray[i] < minimumValueOfRawValues) {
                minimumValueOfRawValues = rawValuesArray[i];
            }
        }

        for (int i = 0; i < rawValuesArray.length; i++) {
            rawValuesArray[i] = rawValuesArray[i] / (float) minimumValueOfRawValues;
            rawValuesArray[i] = Math.round(rawValuesArray[i] * 100.0) / 100.0;
        }
         */
        //print the value set
        /*
    ...
    //assign distances
    private double assignDistances(double val) {
        double temp = 0;
        if (val == 55) {
            temp = 0.3;
        } else if (val == 59) {
    ...
    //average RSSI values
    private double averageRSSI(double val) {
        if (51 <= val && val <= 58) {
            val = 55;
        } else if (54 <= val && val <= 63) {
            val = 59;
        } else if (55 <= val && val <= 65) {
            val = 60;
        } else if (57 <= val && val <= 76) {
            val = 62;
        } else if (61 <= val && val <= 69) {
            val = 66;
        } else if (63 <= val && val <= 73) {
            val = 68;
        } else if (67 <= val && val <= 72) {
            val = 69;
        } else if (67 <= val && val <= 75) {
            val = 71;
        } else if (70 <= val && val <= 80) {
            val = 74;
        } else if (73 <= val && val <= 87) {
            val = 79;
        } else if (75 <= val && val <= 88) {
            val = 81;
        } else if (77 <= val && val <= 90) {
            val = 83;
        } else if (78 <= val && val <= 89) {
            val = 84;
        } else if (78 <= val && val <= 89) {
            val = 85;
        } else if (79 <= val && val <= 92) {
            val = 86;
        } else if (81 <= val && val <= 92) {
            val = 87;
        } else if (82 <= val && val <= 93) {
            val = 88;
        }
        return val;
    }

    //find the greatest common divisor from the value set
    ...
    private double findGCD(double arr[], int n) {
        double result = arr[0];
        for (int i = 1; i < n; i++) {
    ...
    private void initComponents() {
        ...
        getContentPane().add(blackDot, new org.netbeans.lib.awtextra.AbsoluteConstraints(300, 290, 20, 20));
        ...
        getContentPane().add(Map, new org.netbeans.lib.awtextra.AbsoluteConstraints(30, 72, 510, 500));
